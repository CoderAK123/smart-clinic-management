...
// 1. Appointments for a specific doctor in a time range
    List<Appointment> findByDoctorIdAndAppointmentTimeBetween(Long doctorId, LocalDateTime start, LocalDateTime end);

    // 2. Filter appointments by doctor and patient name (case-insensitive) in a time range
   ...
   @Query("UPDATE Appointment a SET a.status = :status WHERE a.id = :id")
    void updateAppointmentStatusById(@Param("status") int status, @Param("id") long id);
